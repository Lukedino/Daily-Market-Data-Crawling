...
> 가격은 yfinance `auto_adjust=True` 조정가다. 배당·분할이 관측된 종목은 그날 전체 이력을 다시 받아 그 종목 행만 교체한다([FEAT-EVENT-REBASE], `backfill_pending.json` 의 `"{market}:rebase"` 키가 재시도 목록). `Dividends`/`Splits` 열은 상수(0/1)이며 실제 액션은 수집 attrs 로만 판정한다.
...
- **남은 것**(같은 검토 — 2026-09-26 현재 코드 기준 재판정, 09-21 코덱스 번호 DM-xx 로 통일): 09-22 `b90caae`·`6121222` 와 09-24 관문 수정으로 **해결** — 백필 `upload_years` 실패 전파(DM-03), 크립토 재조회의 `MarketCap` 보존(DM-06 `_preserve_missing_marketcap`), 손상 파일 strict 읽기·원자 교체(DM-07), 공개 로그 `PublicFormatter`(DM-08), 워크플로 공통 `concurrency`·dry-run 조기 반환(DM-09), 문서·테스트 경로 오탐(DM-13).
  **DM-04 → 2026-09-26 [FEAT-EVENT-REBASE] 로 해소**(이벤트 종목만 전체 이력 재수집·교체, 스키마 불변). **DM-05 → [FIX-US-LOOKBACK] 로 해소**(US 도 1주 재조회, 종목별 실패 목록 대신 창으로 자가 치유). **DM-10 → [FIX-SECTOR-EMPTY-FIELD] 로 해소**. **부분** — DM-11 마지막 저장일이 폴백일(Marcap NaN)이면 월간 재무가 `universe_unverified` 실패 · DM-12 하류(KIS·Mr.Market·ML)의 nullable Int64 소비 미확인.
  참고: "`financials-update` 가 ratios 이력을 매 실행 지운다" 는 2026-09-01 `15bfd12` 에서 **이미 수정**됐다.

| 날짜 | 변경 내용 |
|------|---------|
| 2026-09-26 | **[FIX-SECTOR-EMPTY-FIELD]** DM-10: 야후 `.info` 가 종목 신원(`symbol`/`quoteType`)은 주면서 `sector`/`industry` 만 빠뜨린 응답을 "정상 관측" 으로 저장해 기존 Sector 를 `""` 로 덮고 `updated_at` 까지 새로 찍었다(09-20 BLD·CPRX 류의 404 응답은 09-22 에 잡았지만 이 부분 응답은 남아 있었다). → `collect_sector_meta`: `quoteType` 이 ETF·MUTUALFUND·INDEX·CRYPTOCURRENCY·CURRENCY·FUTURE·OPTION(`_SECTORLESS_QUOTE_TYPES`)이면 빈 섹터가 정상, **주식이거나 종류를 모르면 빈 값은 실패 필드**로 표시해 `save_sector_meta` 가 기존 값·시각을 보존(신규 종목은 종전대로 빈 채 게시). 테스트 +5(변이 1종 검출), 637 passed. 09-27 10:00 sector-meta 첫 실행에 적용 |
| 2026-09-26 | **[FIX-US-LOOKBACK]** DM-05: 공급자가 매일 소수 종목(실측 0.47%)을 돌려주지 않고 그 누락은 허용되는데(`collection_incomplete_tolerated`), US 증분 창이 직전 저장 세션 하나뿐(`start = last_date`)이라 커서가 그 종목의 날짜를 지나면 다시 요청되지 않아 **종목 단위 영구 구멍**이 됐다 — 크립토 08-31 결손과 같은 결함. → `US_LOOKBACK_DAYS = 7`(`incremental_start_date`): US 도 최근 1주를 매번 다시 받아 그 사이 공급자가 돌려준 봉이 `(Ticker, Date)` keep="last" 병합으로 채워진다(요청 수 동일, 봉 수만 5배). 부수 수정: 창이 넓어지면 **포기한 결손 날짜**(`sparse_session_abandoned`)가 매일 창 안에 남는데 `_incremental_cursor` 가 그걸 다시 붙잡아 커서가 영영 멈추는 회귀가 있어, 커서가 이미 지난 날짜(`day <= last_date`)는 보류 대상에서 뺐다(그 날짜는 채워지면 병합으로 들어오고, 여전히 희소하면 게시에서만 빠진다). 테스트 3건 추가·1건 재고정(변이 2종 검출), 631 passed. 종목별 실패 목록 영속화는 두지 않았다 — 일주일 넘게 안 오는 종목은 사실상 상장폐지 |
| 2026-09-26 | **[FEAT-EVENT-REBASE]** DM-04(사용자 결정 "이벤트 종목 전체 재수집"): US/Crypto 증분은 `auto_adjust=True` 라 배당·분할 종목의 과거 봉이 소급 재조정되는데 파일에는 옛 기준 행이 남아 이력 안에서 기준이 갈라졌다(09-24 `4692ad2` 가 그 종목을 overlap 비교에서 면제만 함). → `update_market()` 이 관측된 `action_tickers` 를 돌려주고, `run_ohlc_update` 가 증분 게시 **뒤에** `rebase_action_tickers(market, ...)` 를 부른다: ① 대상을 `backfill_pending.json` 의 `"{market}:rebase"` 키에 **먼저 게시**(attrs 는 휘발성 — 중간에 죽어도 다음 실행이 안다; SA 가 Drive 에 새 파일을 못 만들어 같은 파일을 쓴다) ② 기준 파일이 **있는** 연도만(`ensure_year_baselines` 3상태, `absent` 는 새 파일을 만들지 않음) 전체 이력을 받고 ③ 종목별로 **모든 연도를 받은 뒤** 판정 — 하드 실패, 기존 행이 있는 연도가 비거나 90% 미만이면 어느 연도도 건드리지 않고 보류(`rebase_deferred`, 다음 실행 재시도) ④ 교체는 `validate_price_basis(replace_tickers=)` + `save_year(replace_tickers=)`, 교체가 기존 행을 먼저 걷어내 `save_year` 의 시총 보존이 닿지 않으므로 `_preserve_missing_marketcap` 으로 시총 이력을 직접 잇는다 ⑤ 커서 불변, 하드 실패는 `rebase_incomplete` 로 종료(알림). 비용: 이벤트 종목이 있는 날 연도당 조회 1회 + 그 연도 파일 재업로드. 테스트 `tests/test_ohlc_rebase.py` 11건(변이 4종 검출), 630 passed. 첫 실 실행에서 로그 `rebase_started`·`rebase_completed`/`rebase_deferred` 와 pending 키를 확인할 것 |
